    ...
    <div className="min-h-screen" style={{ backgroundColor: '#FFF9E6' }}>
      ...
      <main className="container mx-auto px-6 py-12">
        <div className="max-w-4xl mx-auto">
          <div style={{ border: '8px solid #1e293b', borderRadius: '24px', padding: '48px', backgroundColor: '#ffffff', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
            ...
            <div>
              {currentStep === 1 && (
                <div>
                    ...
                    {recipients.map((recipient, idx) => (
                      <div key={recipient.id} className="flex gap-3 items-center">
                        ...
                        <input
                          ...
                          className="flex-1 px-4 py-3 border-2 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none"
                        />
                        {recipients.length > 1 && (
                          <button
                  ...
                  <button
                    ...
                    className="w-full mt-6 py-4 bg-purple-600 text-white rounded-lg font-bold text-lg hover:bg-purple-700 disabled:bg-slate-300 disabled:cursor-not-allowed"
                  >
                    Continue to Amounts →
                  </button>
                </div>
              ...
              {currentStep === 2 && (
                <div>
                  ...
                  <div className="space-y-4 mb-6">
                    {recipients.map((recipient, idx) => (
                      <div key={recipient.id} className="border-2 rounded-lg p-4">
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="flex-1 px-4 py-3 border-2 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none"
                          />
                          <span className="text-slate-600 font-bold">ETH</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 py-4 border-2 border-slate-300 rounded-lg font-bold text-lg hover:bg-slate-50"
                    >
                      ← Back
                    </button>
                    <button
                      ...
                      className="flex-1 py-4 bg-purple-600 text-white rounded-lg font-bold text-lg hover:bg-purple-700 disabled:bg-slate-300 disabled:cursor-not-allowed"
                    >
                      Review & Send →
                    </button>
                  </div>
                  ...
                  <div className="space-y-4 mb-6">
                    <div className="bg-slate-50 rounded-lg p-6">
                      <div className="grid grid-cols-2 gap-6">
                        <div>
                          <div className="text-sm text-slate-600 mb-1">Recipients</div>
                          <div className="text-3xl font-bold">{recipients.length}</div>
                        </div>
                        <div>
                          <div className="text-sm text-slate-600 mb-1">Total Amount</div>
                          <div className="text-3xl font-bold">{calculateTotal().toFixed(4)} ETH</div>
                        </div>
                        <div>
                          <div className="text-sm text-slate-600 mb-1">Protocol Fee (0.3%)</div>
                        ...
                        <div>
                          ...
                          <div className="text-2xl font-bold text-purple-600">{(calculateTotal() * 1.003).toFixed(4)} ETH</div>
                        </div>
                      </div>
                    </div>
                  ...
                  {isSuccess && (
                    <div className="mt-6 p-4 bg-green-50 border-2 border-green-200 rounded-lg">
                      ...
                      <a 
                        ...
                        target="_blank"
                        className="text-sm text-green-600 underline mt-1 block font-medium"
                      >
                        View on BaseScan →
                      </a>
                    </div>
